...
abstract class TyClass(override val className: String,
    ...
    override fun processAlias(processor: Processor<String>): Boolean {
        ...
        if (!isGlobal && !isAnonymous /*&& LuaSettings.instance.isRecognizeGlobalNameAsType*/) {
            val aliasName = getGlobalTypeName(className)
            if (aliasName != className)
                return processor.process(aliasName)
        }
        return true
    }
    ...
    override fun processMembers(context: SearchContext, processor: (ITyClass, LuaClassMember) -> Unit, deep: Boolean) {
        ...
        val all = mutableListOf<LuaClassMember>()
        val list = memberIndex.get(clazzName, project, GlobalSearchScope.allScope(project))
        all.addAll(list)

        processAlias(Processor { alias ->
            val classMembers = memberIndex.get(alias, project, GlobalSearchScope.allScope(project))
    ...
    companion object {
        ...
        fun createAnonymousType(nameDef: LuaNameDef): TyClass {
            val tyName = getAnonymousType(nameDef)
